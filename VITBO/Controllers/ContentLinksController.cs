...
namespace VITBO.Controllers
{
    ...
    public class ContentLinksController : Controller
    {
        ...
        public ContentLinksController(IMediaService mediaService)
        {
        ...
        public async Task<IActionResult> Index(int? contentId)
        {
            ...
            var list = await _mediaService.GetContentLinksAsync(contentId, token, userAgent);
            return View(list);
        }
        ...
        {
            return View(new ContentLinkDto { ContentId = contentId ?? 0, LangID = 1 });
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(ContentLinkDto model)
        {
            if (ModelState.IsValid)
            {
                var token = HttpContext.User.FindFirst("JWToken")?.Value ?? HttpContext.Session.GetString("JWToken") ?? string.Empty;
                var userAgent = GetUserAgent() ?? string.Empty;
                var success = await _mediaService.CreateContentLinkAsync(model, token, userAgent);
                if (success)
                {
                    return RedirectToAction(nameof(Index), new { contentId = model.ContentId });
                }
                ModelState.AddModelError("", "Failed to add link.");
            }
            return View(model);
        }
        ...
        public async Task<IActionResult> Delete(int id, int contentId)
        {
            var token = HttpContext.User.FindFirst("JWToken")?.Value ?? HttpContext.Session.GetString("JWToken") ?? string.Empty;
            var userAgent = GetUserAgent() ?? string.Empty;
            await _mediaService.DeleteContentLinkAsync(id, token, userAgent);
