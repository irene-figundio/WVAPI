...
namespace VITBO.Services
{
    public class ExpertsService : IExpertsService
    {
        ...
        public async Task<List<ExpertDto>> GetExpertsAsync(int langId, string token, string userAgent)
        {
            ...
            var endpoint = $"{apiBase}/api/experts?langId={langId}";
        ...
        {
            var apiBase = _configuration["ApiBaseAddress"] ?? "https://localhost:7275";
            var endpoint = $"{apiBase}/api/experts/{id}?langId={langId}";
            var result = await _httpService.SendHttpRequestAsync(HttpMethod.Get, endpoint, token, userAgent, null);
        ...
        {
            var apiBase = _configuration["ApiBaseAddress"] ?? "https://localhost:7275";
            var endpoint = $"{apiBase}/api/experts";
        ...
        {
            var apiBase = _configuration["ApiBaseAddress"] ?? "https://localhost:7275";
            var endpoint = $"{apiBase}/api/experts/{id}";
            var jsonContent = JsonSerializer.Serialize(expert);
            return await _httpService.SendHttpRequestAsync(HttpMethod.Put, endpoint, token, userAgent, jsonContent) is HttpResponseMessage response && response.IsSuccessStatusCode;
        }
