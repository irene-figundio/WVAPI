...
namespace AI_Integration.Controllers
{
    ...
    public class VariantPricesController : ControllerBase
    {
        ...
        public VariantPricesController(IUnitOfWork unitOfWork)
        {
        ...
        {
            var log = WebApiLogHelper.NewLog("GET", "api/variantprices", $"eventId={eventId}", userAgent, "List variantprices");
            var sw = Stopwatch.StartNew();
            try
            {
                var query = _unitOfWork.Query<VariantPrice>().Where(e => e.IsDeleted != true);
                if (eventId.HasValue)
                {
        ...
        {
            var log = WebApiLogHelper.NewLog("GET", $"api/variantprices/{id}", "", userAgent, "Get variantprice by id");
            var sw = Stopwatch.StartNew();
            try
            {
                var item = await _unitOfWork.GetByIdAsync<VariantPrice>(id);
        ...
        [HttpPost]
        public async Task<IActionResult> Add([FromBody] VariantPrice item, [FromHeader(Name = "User-Agent")] string userAgent = "")
        {
            var log = WebApiLogHelper.NewLog("POST", "api/variantprices", item?.ToString(), userAgent, "Create variantprice");
            var sw = Stopwatch.StartNew();
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Put(int id, [FromBody] VariantPrice changes, [FromHeader(Name = "User-Agent")] string userAgent = "")
        {
            var log = WebApiLogHelper.NewLog("PUT", $"api/variantprices/{id}", changes?.ToString(), userAgent, "Update variantprice");
            var sw = Stopwatch.StartNew();
            if (!ModelState.IsValid) return BadRequest(ModelState);
            try
            {
        ...
        {
            var log = WebApiLogHelper.NewLog("DELETE", $"api/variantprices/{id}", "", userAgent, "Delete variantprice");
            var sw = Stopwatch.StartNew();
            try
            {
                var item = await _unitOfWork.GetByIdAsync<VariantPrice>(id);
