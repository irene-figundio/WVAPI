...
namespace AI_Integration.Controllers
{
    ...
    public class TripMustsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Add([FromBody] TripMust must, [FromHeader(Name = "User-Agent")] string userAgent = "")
        {
            ...
            var sw = Stopwatch.StartNew();
            if (must == null) return BadRequest(new { success = false, message = "Trip must info is required." });

            if (must.TripId <= 0) ModelState.AddModelError("TripId", "TripId is required.");
            if (string.IsNullOrWhiteSpace(must.Text)) ModelState.AddModelError("Text", "Text is required.");
            if (must.TypeId <= 0) ModelState.AddModelError("TypeId", "TypeId is required (1: Inclusion, 2: Exclusion).");
